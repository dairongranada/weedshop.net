...
var isAdvancedUpload = function () {
  ...
  uploadButton.addEventListener("click", () => {
    ...
    if (isFileUploaded != '') {
        ...
        $.ajax({
            ...
            console.log(data);
            existing = []; non_existent = []; 
            existing.push(data.existing_employees);
            non_existent.push(data.non_existent_employees);
            generarDatos();

            $('#btn-excelCindy').remove();


            const nuevaFila = $(`
            <div id='btn-excelCindy' class="d-flex flex-row justify-content-flex-end" style='width:100%' >
                <button class="btn btn-primary" type="button" onclick="generarExcel()">
                <i class="fa-solid fa-file-excel"></i> Generar Excel</button>
            </div>
            `);

            $(".connot-button").append(nuevaFila);
  ...
  const generarDatos = () => {
    ...
    let infodateexport = [
        ["COINCIDENCIA","CEDULA", "NOMBRE EMPLEADO EX-EMPLEADO", "FECHA DE RETIRO", "OBSERVACIONES", 'AVANCE'],
    ]
...
const generarExcel = () => {
    ...
    var worksheet  = XLSX.utils.aoa_to_sheet(generarDatos());
  
    XLSX.utils.book_append_sheet(workbook, worksheet, `Consulta_masiva`);
    
    /* Descargar el archivo Excel */
    var nombreArchivo = `Consulta_masiva N.xlsx`;
